...
print (yellow("Call fron anoter contract of setFeeReciever with no admin address setted"))
Wallet.call_method(
        "sendTransaction",
    {
        ...
        "payload":ts4.encode_message_body(
            "LLCService", "setFeeReciever", {"feeReciever": Wallet.address})
    },
    keys_Multisig[0]
)
ts4.dispatch_one_message(expect_ec=100)
# Проверяем что фии ресивер не засеччен
...
print (yellow("Call from anoter contract of setFeeReciever with admin address setted"))
Wallet.call_method(
        "sendTransaction",
    {
        ...
        "flags": 0,
        "payload":ts4.encode_message_body(
            "LLCService", "setFeeReciever", {"feeReciever": Wallet2.address})
    },
    keys_Multisig[0]
)
ts4.dispatch_one_message()
# Проверяем засеченый фии ресивер
assert (LLCServiceAdmin.call_getter("getExt")==(0, 0, Wallet.address, Wallet2.address, 100000000000, 0))
print (green("Success: fee reciever setted"))

# Тест 3 с неверным валью
print (yellow("Call from anoter contract of setFeeReciever with value 2000 and admin address setted"))
Wallet.call_method(
        "sendTransaction",
    {
        "dest": LLCServiceAdmin.address,
...
ts4.dispatch_one_message()
# Проверяем что ресивер не засеччен
assert (LLCServiceAdmin.call_getter("getExt")==(0, 0, Wallet.address, Wallet2.address, 100000000000, 0))
print (green("Success: fee reciver not setted"))
